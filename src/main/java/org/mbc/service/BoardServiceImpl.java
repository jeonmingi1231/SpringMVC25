...
@AllArgsConstructor // 생성자가 자동 주입 ( new BoardServiceImpl(mapper);
@Log4j2 // 로깅 처리용
public class BoardServiceImpl implements BoardService{
	...
	public BoardVO get(Long bno) {
		// 게시물의 번호가 들어가면 메퍼의 select문이 동작 해야한다.
	...
	public boolean modifiy(BoardVO board) {
		// 프론트가 객체가 넘어오면 매퍼를 통해서 update 쿼리가 실행된다.
	...
	public boolean remove(Long bno) {
		// 프론트가 객체가 넘어오면 매퍼를 통해서 delete 쿼리가 실행된다.
