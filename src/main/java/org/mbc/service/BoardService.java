...
public interface BoardService {
	...
	// 인터페이스를 사용하려면 구현클래스를 만들고 실행문을 넣게 된다.
	// 인터페이스를 사용하는 이유는 조장이 메서드명을 강제로 셋팅 하기 위함이다.

	public void register(BoardVO board); // 조장이 만든 메서드 명.(시그니처)
	// 게시판 등록용 메서드
	...
	// 게스글 수정 하기(객체가 들어가서 수정이 됐는지 t/f)

	public boolean remove(Long bno);
	// 게시글 삭제하기(번호가 들어가서 삭제 됐는지 t/f)

	public List<BoardVO> getList();
	// 이 메서드가 호출되면 리스트 타입으로 BoardVO객체 들이 넘어온다.
